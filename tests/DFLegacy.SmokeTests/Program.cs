...
var rejectionSamples = new Queue<ulong>([5, 16]);
bool TryGetRejectionSample(out ulong value)
{
    value = rejectionSamples.Dequeue();
    return true;
}

var rejectionRandom = new GameRandomSource(
    cpuSupportsRdSeed: true,
    nativeBridgeLoaded: true,
    rdSeedEnabled: true,
    TryGetRejectionSample,
    _ => throw new InvalidOperationException("OS fallback must not be used"),
    _ => throw new InvalidOperationException("PRNG fallback must not be used"));
var rejectionValue = rejectionRandom.Next(10);
var rejectionStatus = rejectionRandom.GetStatus();
Check(rejectionValue == 6
    && rejectionStatus.RdSeedSamples == 1
    && rejectionStatus.RejectedSamples == 1
    && rejectionStatus.OperatingSystemSamples == 0,
    "game random source rejects modulo-biased RDSEED samples before mapping a bounded value");

bool FailRdSeed(out ulong value)
{
    value = 0;
    return false;
}

var operatingSystemFallbackRandom = new GameRandomSource(
    cpuSupportsRdSeed: true,
    nativeBridgeLoaded: true,
    rdSeedEnabled: true,
    FailRdSeed,
    _ => 7,
    _ => throw new InvalidOperationException("PRNG fallback must not be used"));
Check(operatingSystemFallbackRandom.Next(10) == 7
    && operatingSystemFallbackRandom.GetStatus() is
    {
        RdSeedUnavailable: 1,
        OperatingSystemSamples: 1,
        PseudoRandomSamples: 0
    },
    "game random source falls back to the OS CSPRNG when RDSEED has no seed available");

var pseudoRandomFallback = new GameRandomSource(
    cpuSupportsRdSeed: false,
    nativeBridgeLoaded: false,
    rdSeedEnabled: false,
    FailRdSeed,
    _ => throw new CryptographicException("simulated unavailable OS CSPRNG"),
    _ => 3);
Check(pseudoRandomFallback.Next(10) == 3
    && pseudoRandomFallback.GetStatus() is
    {
        RdSeedSamples: 0,
        OperatingSystemSamples: 0,
        PseudoRandomSamples: 1
    },
    "game random source reaches the ordinary PRNG only when hardware and OS entropy are unavailable");

var sharedRandomValue = GameRandomSource.Shared.Next(10_000);
var sharedRandomStatus = GameRandomSource.Shared.GetStatus();
Check(sharedRandomValue is >= 0 and < 10_000
    && (!sharedRandomStatus.RdSeedEnabled
        || sharedRandomStatus is
        {
            CpuSupportsRdSeed: true,
            NativeBridgeLoaded: true,
            PreferredProvider: "RDSEED"
        })
    && sharedRandomStatus.RdSeedSamples
        + sharedRandomStatus.OperatingSystemSamples
        + sharedRandomStatus.PseudoRandomSamples >= 1,
    "live game random source reports a valid hardware or fallback provider");
